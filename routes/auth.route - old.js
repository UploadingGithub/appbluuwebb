...
import { miValidationResult } from '../middlewares/miValidationResult.js';
...
custom(function)
    - function: (value, { req }) => { ... }
            - Recibe 2 parámetros:
                    - value -> el valor del primer parámetro: campo, del body (al que está encadenado custom) (en este ejemplo: password) ------------------> (1)
...
// =============================================================================================================================
// Ahora pongo la cadena de manejadores/controladores/middlewares sin []: sólo separados con , (también se puede así)
router.post(
    "/register",
    ...
    body('password', 'Formato de password incorrecto.') // <------------------------------------------------------------------------------------------------- (1)
    ...
    // -------------------------------------------------------------------------------------
    miValidationResult, // Si dieron error: no pasar el control a register, y devolver error.
...
router.post(
    ...
    // Comprobar si las validaciones dieron algún error.
    // -------------------------------------------------------------------------------------
    miValidationResult, // Si dieron error: no pasar el control a login, y devolver error.
...
// Al ser exportado por default, al importarlo se puede importar con el nombre que se quiera.
